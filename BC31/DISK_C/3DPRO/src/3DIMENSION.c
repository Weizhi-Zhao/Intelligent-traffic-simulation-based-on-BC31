...
void main()
{
	...
	bodyInfo body1;
	bodyInfo body2;
	bodyInfo road1, road2;
    Connection *c = newConnection("keyboard", &coordinate, ChangeView);
	DirectedLine *line1 = newDirectedLine(13, 2, 4, 2);
	DirectedLine *nowLine = line1;
	DLEdge *it;
	DirectedLine *line2 = newDirectedLine(4, 2, 2, 4);
	DirectedLine *line3 = newDirectedLine(2, 4, 2, 15);
	DirectedLine *line4 = newDirectedLine(4, 2, 0, 0);
	DirectedLine *line5 = newDirectedLine(0, 0, -2, -4);
	DirectedLine *line6 = newDirectedLine(-2, -4, -2, -15);
	DirectedLine *line7 = newDirectedLine(4, 2, -15, 2);
	ConnectDLine(line1, line7);
	ConnectDLine(line1, line2);
	ConnectDLine(line2, line3);
	ConnectDLine(line1, line4);
	ConnectDLine(line4, line5);
	ConnectDLine(line5, line6);
    ...
    BodyInfoInit(&body1, 2, 2, 4, 0, 0, -6.2, 1, 0, 0);
    BodyInfoInit(&body2, 2, 2, 2, 3, 0, -2.2, 1, 0, 0);
	BodyInfoInit(&road1, 8, 30, 1, 0, 0, 0, 1, 0, 0);
	BodyInfoInit(&road2, 30, 8, 1, 0, 0, 0, 1, 0, 0);
   
    CoordinateInit(&coordinate);
	coordinate.maganification = 6;
    ...
    while(1)
    {
		//dis += v * (time(NULL) - timer);
		//timer = time(NULL);
		dis += 1;
		while(dis > nowLine->lenth)
		{
			dis = dis - nowLine->lenth;
			if(nowLine->branchNum == 0)
			{
				nowLine = line1;
			}
			else
			{
				k = rand() % nowLine->branchNum;
				it = nowLine->next;
				for(i = 0; i < k; i++)
				{
					it = it->next;
		...
		pos = posToPoint(nowLine, dis);
		BodyInfoInit(&body2, 2, 2, 2, pos.x, pos.y, -2.2, 1, 0, 0);
    	clearFrameBuffer();
		for(i = 0;i < 8;i++)
		{
			...
			VectorSet(road1.finalCoordinate[i], 0, 0, 0, 0);
	    	CoordinateChange(road1.originCoordinate[i], coordinate.change, road1.finalCoordinate[i]);
			VectorSet(road2.finalCoordinate[i], 0, 0, 0, 0);
	    	CoordinateChange(road2.originCoordinate[i], coordinate.change, road2.finalCoordinate[i]);
			
		}

		ConnectDots(&road2, &coordinate, 14);
		ConnectDots(&road1, &coordinate, 14);
		ConnectDots(&body2, &coordinate, 7);
		ConnectDots(&body1, &coordinate, 3);
...
{	
	...
	for(i = 0;i < 6;i++)
	{
		...
		{
			DrawPlane(&(body->planeInformation[i]), coordinate, color);
		}
	}
}
...
{
	putParallelogram(plane->finalDotID[0][0] * coordinate->maganification + 320
	, plane->finalDotID[0][1] * coordinate->maganification + 240
	, plane->finalDotID[1][0] * coordinate->maganification + 320
	, plane->finalDotID[1][1] * coordinate->maganification + 240
	, plane->finalDotID[2][0] * coordinate->maganification + 320
	, plane->finalDotID[2][1] * coordinate->maganification + 240
	, plane->finalDotID[3][0] * coordinate->maganification + 320
	, plane->finalDotID[3][1] * coordinate->maganification + 240, color);
