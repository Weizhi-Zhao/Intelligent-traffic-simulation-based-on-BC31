...
void InitCar(Car *car, float speed)
{
    car->turn = 0;
    car->visiable = 0;
    car->acc = speed / 3;
    car->maxSpeed = speed;
    car->slowSpeed = speed / 3;
...
void InitBus(Car *car, float speed)
{
    car->turn = 0;
    car->visiable = 0;
    car->acc = speed / 3;
...
void RunCar(Car car[], int carnum, float carSpeed, float busSpeed, DirectedLine *startLine[], coordinateSystem *coordinate)
{
        ...
        if(car[i].visiable == 0)
        {
            ...
            if(k == 0)
            {
                ...
                if(state == 0)
                {
                    if(rand() % 2 == 0)
                    {
                        InitCar(&car[i], carSpeed);
                    }
                    else
                    {
                        InitBus(&car[i], busSpeed);
                    }
                    car[i].visiable = 1;
                    car[i].turn = 0;
